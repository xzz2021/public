...
#### 整体流程
  ...
  5. 前端轮询状态, 成功跳转页面
  6. 微信的货币单位是分,所以金额必须是正整数
下图包含完整支付(订单支付/补差价)/退款(全额/部分)
![流程图](https://github.com/xzz2021/public/blob/main/image/markdown/wxpay.png?raw=true "mind")
...
3. 核心代码
   ...
   ```ts
    ...
    generateAuthorization(type: string = 'native', body: Partial<WxPayData> | WxRefundData = {}) {
      ...
      if (type === 'refund') {
        newBody.notify_url = this.refund_notify_url;   // 退款回调地址
    ...
    //  获取微信官方 返回的 商户自己的  平台证书   并存储publicKey
      ...
      async verifySign(params: { timestamp: string | number; nonce: string; serial: string; signature: string; body: Record<string, any> | string }) {
        ...
        if (!isVerify) throw new Error('签名验证失败'); 
      }
    ...
    async wechatRefund(objdata: WxRefundData) {
        ...
        try {
          ...
          const errorMessage = errorData?.message || errorData?.code || '未知错误';
          // console.log('🚀 ~ WxPay ~ wechatRefund ~ errorData:', errorData);
          throw new Error(`发起微信退款失败, 原因: ${errorMessage}`);
        }
      }
...
3. 签名验证需要: 先调用官方接口拿到自己的所有证书(或许可以本地读取跳过???), 进行解密后再以 `{序列号:publicKey}`形式存储好,后续校验公匙进行签名验证
...
5. 退款
   ...
   ```ts
    const refundDataFinal = {
      transaction_id: trade_no,  //  微信平台交易id
      out_refund_no: refundNo,  //  自定义退款单号
      //   adjustedNet  退款金额     //   rawOrderAmount 原订单支付金额
